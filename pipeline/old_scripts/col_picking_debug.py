...
def find_approx_minima(sums, number):
	fig, ax = plt.subplots()
...
def find_minima(sums,number,threshold=20):
	'''
	...
	for l,r in bounds:
		...
		ax.plot(mid_point,sums[mid_point],'bo')
...
def find_grid(img):
	...
	type_totals.append('row')
	sum_totals.append(sums_y)
	type_totals.append('col')
	sum_totals.append(sums_x)
...
def find_reference(image):
	...
	'''
	# The agar outline
	all_points = [[19,27],[377,27],[391,605],[19,595]]
...
def pick_colonies():
	...
	type_totals = []
	plate_totals = []
	for target_plate in session.query(Plate).join(Build,Plate.builds).filter(Build.status == 'building').order_by(Build.build_name):
		build_name = target_plate.builds.build_name
		path = '{}/builds/{}/{}_trans_pics'.format(BASE_PATH,build_name,build_name)
		for file in sorted(glob.glob('{}/*.jpg'.format(path))):
			plate_name = file.split("/")[-1].split('.')[0]
			print(plate_name)
			print("Current image: ",file)
			plate_totals += [plate_name,plate_name]
			skip = input('Skip? (y/n): ')
			if skip == 'y':
				continue
			print("Inoculate: ",args.inoculate)
			find_colony_coordinates(file,args.check)
